...
type ConfigWrapper struct {
	...
	blockedDeviceMap map[protocol.DeviceID]bool
...
func (w *ConfigWrapper) BlockedDevices() map[protocol.DeviceID]bool {
	w.mut.Lock()
	defer w.mut.Unlock()
	if w.blockedDeviceMap == nil {
		w.blockedDeviceMap = make(map[protocol.DeviceID]bool, len(w.cfg.BlockedDevices))
		for _, dev := range w.cfg.BlockedDevices {
			w.blockedDeviceMap[dev] = true
		}
	}
	return w.blockedDeviceMap
}
